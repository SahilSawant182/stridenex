...
export default function Sidebar({ role, collapsed = false, onToggle, isMobile, onClose }: SidebarProps) {
  ...
  return (
    <motion.div
      ...
      className={`border-r flex flex-col h-screen sticky top-0 z-50 shrink-0 transition-colors duration-300 ${
        isShortsPage ? 'bg-[#0f0f0f] border-zinc-800' : 'bg-white border-slate-200'
      }`}
    >
      {/* Logo Area */}
      <div className={`h-[72px] shrink-0 flex items-center justify-between px-4 border-b transition-colors duration-300 ${
        isShortsPage ? 'border-zinc-800' : 'border-slate-100'
      }`}>
        ...
        {!isMobile && onToggle && (
          <button
            onClick={onToggle}
            className={`p-1.5 rounded-full border shadow-sm transition-all duration-300 z-10 ${
              isShortsPage
