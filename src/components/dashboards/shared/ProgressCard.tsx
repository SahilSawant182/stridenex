                ...
                {items.map((item, index) => (
                    <div key={item.label}>
                        <div className="flex justify-between items-center mb-1">
                            ...
                            <span className={cn("text-xs font-semibold", textColorMap[item.color as keyof typeof textColorMap] || "text-slate-800")}>
                                {item.subtitle || item.value}
                            </span>
                        </div>
                        <Progress
                            value={item.percentage || item.value}
                            className={cn("h-1.5", colorMap[item.color as keyof typeof colorMap] || "bg-emerald-500")}
                        />
                    </div>
                ))}
            </div>
