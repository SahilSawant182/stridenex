        ...
        <div className="w-64 flex-shrink-0 border-r border-slate-200 bg-white flex flex-col hidden md:flex overflow-y-auto">
          ...
          <div className="p-4 border-t border-slate-200">
            <div className="flex items-center justify-between mb-3">
              ...
              <Plus className="w-3 h-3 text-slate-400 cursor-pointer hover:text-slate-700" onClick={() => setIsCategoryModalOpen(true)} />
            </div>
            <div className="space-y-1">
              {categories.length > 0 ? (
          ...
          <div className="p-4 border-t border-slate-200">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center gap-2">
                <Plus className="w-3 h-3 text-slate-400 cursor-pointer hover:text-slate-700" onClick={() => setIsTagModalOpen(true)} />
            ...
            <div className="flex flex-wrap gap-2">
              {tags.length > 0 ? (
                tags.map((tag: any, idx: number) => (
                  <span key={idx} className="px-2 py-1 bg-slate-50 text-slate-650 rounded-md text-[10px] font-semibold border border-slate-200 cursor-pointer hover:text-slate-900 hover:bg-slate-100 transition-colors">
                    #{tag.title || tag.name}
                  </span>
                ))
              ) : (
                  ...
                  <div className="space-y-6 max-w-4xl mx-auto">
                    ...
                    <div className="flex gap-4 bg-white p-6 rounded-2xl border border-slate-200/60 shadow-sm">
                      ...
                      <div>
                        ...
                        <p className="text-base text-slate-650 whitespace-pre-wrap mt-2">{postDetails.content}</p>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      {postDetails.comments && postDetails.comments.length > 0 ? (
                        <div className="space-y-4">
                          {postDetails.comments.map((comment: any, idx: number) => (
                            <div key={idx} className="bg-white p-5 rounded-xl border border-slate-200/60 shadow-sm">
                              ...
                              <p className="text-sm text-slate-650 mb-3">{comment.content}</p>
                        ...
                        <p className="text-sm text-slate-450 italic">No comments yet.</p>
                      )}
                    </div>
                  </div>
                ) : (
                  ...
                  <div className="max-w-4xl mx-auto w-full">
                    ...
                    <div className="flex items-center gap-3">
                      <input
                        ...
                        className="flex-1 bg-slate-50 border border-slate-200 rounded-lg p-3 text-sm text-slate-850 placeholder:text-slate-400 focus:outline-none focus:bg-white focus:border-[#FF6B00] transition-all"
                      />
                      <button
            ...
            <div className="flex flex-col h-full bg-slate-50">
              ...
              <div className="flex items-center gap-4 px-6 py-4 border-b border-slate-200 bg-white shadow-sm">
                ...
                <div className="flex items-center gap-3 border-l border-slate-200 pl-4">
                  ...
                  <div>
                    <h2 className="text-lg font-bold text-slate-850 leading-tight">{selectedCategory.category_name || selectedCategory.name}</h2>
                  </div>
                </div>
              </div>
                  ...
                  posts.map((post: any, idx: number) => (
                    ...
                    >
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-slate-650 whitespace-pre-wrap mb-3 leading-relaxed">{post.content}</p>
                ...
                <div className="w-full flex items-center gap-3">
                  <div className="flex-1 bg-slate-50 border border-slate-200 rounded-xl overflow-hidden focus-within:bg-white focus-within:border-[#FF6B00] transition-all h-[48px]">
                    <textarea 
                      ...
                      className="w-full h-full bg-transparent text-sm text-slate-850 px-4 py-3 focus:outline-none resize-none placeholder:text-slate-400"
                    />
                  </div>
                  <Button 
