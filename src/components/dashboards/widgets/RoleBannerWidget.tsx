      ...
      <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex-1">
          ...
          <p className="text-xs text-slate-500 leading-relaxed max-w-sm">
            Invite other campuses to the platform. For every college that joins, your institution earns <span className="font-bold text-emerald-600">1000 Points</span>, and they get a <span className="font-bold text-emerald-600">500 Points</span> bonus!
          </p>
        </div>
...
export default function RoleBannerWidget({ role, customData, onlyModal = false }: RoleBannerWidgetProps) {
  ...
  const industryFields: DynamicField[] = useMemo(() => {
    ...
    const refCode = customData?.rawIndustryData?.user_details?.referal_code || customData?.rawIndustryData?.referal_code;
    if (refCode) {
      fields.push({
        name: "referal_code",
        label: "Your Referral Code",
        ...
        customRender: (formData: any) => {
          return (
            <div className="flex flex-col w-full gap-4">
              <ReferralCodeDisplay referal_code={refCode} showToast={showToast} />
              <ReferralPerformanceWidget referralCode={refCode} role="industry" />
            </div>
          );
        }
      });
