    ...
    <div className="relative overflow-hidden rounded-2xl border border-emerald-100 bg-gradient-to-br from-emerald-50 to-teal-50 shadow-sm p-5 mt-2">
      ...
      <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex-1">
          ...
          <h4 className="text-sm font-bold text-slate-800 mb-1">Grow your network! 🌐</h4>
          <p className="text-xs text-slate-500 leading-relaxed max-w-sm">
            Share your code with students, mentors, colleges, or companies. For every referral that joins, earn <span className="font-bold text-emerald-600">500 Points</span>!
          </p>
        </div>
        <div className="flex items-center gap-2 bg-white p-1.5 rounded-xl border border-slate-200 shadow-sm shrink-0">
          ...
          <button
            type="button"
            onClick={() => { navigator.clipboard.writeText(referal_code); setCopied(true); showToast("Referral code copied!", "success"); setTimeout(() => setCopied(false), 2000); }}
...
export default function MyProfileTabContent() {
  ...
  ], [modalValues.state, modalValues.district, modalValues.tahsil, mentorData?.user_details?.referal_code, mentorData?.referal_code]);
  ...
  const modalInitialValues = useMemo(() => {
    ...
    return {
      ...
      referal_code: mentorData.user_details?.referal_code || mentorData.referal_code || "",
    };
  }, [mentorData]);
